...
class Solution:
    ...
    def backtrack(self,grid,i,j,visited,sum):
        if i<0 or i>=len(grid) or j<0 or j>=len(grid[0]) or visited[i][j] or grid[i][j]==0:
            return 
        cursum = sum+grid[i][j]                     #每次累加，并更新最大值
        self.re = max(cursum, self.re)
        visited[i][j] = True
        self.backtrack(grid,i-1,j,visited,cursum) 
        self.backtrack(grid,i+1,j,visited,cursum) 
        self.backtrack(grid,i,j-1,visited,cursum) 
        self.backtrack(grid,i,j+1,visited,cursum)
        visited[i][j] = False 
